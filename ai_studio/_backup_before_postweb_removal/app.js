const modes = {
  ...
  "post-tiktok-web": {
    "title": "Post TikTok(Web)",
    "eyebrow": "TikTok web publishing",
    "desc": "เตรียมโพสต์ผ่าน TikTok Web พร้อม caption, hashtag, เวลาโพสต์ และสถานะอัปโหลด"
  },
  "post-tiktok-mobile": {
...
const PLATFORM_UNIT_MODES = {
  ...
  "tiktok-post-web": { title: "Post TikTok(Web)", eyebrow: "TikTok / Web Post", desc: "TikTok web post" },
...
const PLATFORM_UNITS = {
  tiktok: [
    ...
    { route: "tiktok-post-web", icon: "public", label: "Post TikTok(Web)" }
  ],
  shopee: [
...
function isTikTokPostWebRoute(route) {
  return String(route || "") === "tiktok-post-web";
}
...
function setRoute(route) {
  ...
  document.body.classList.toggle("post-web-route", !!mode && isTikTokPostWebRoute(route));
  ...
  const tiktokPostWebRoutePanel = document.getElementById("tiktokPostWebPanel");
  ...
  if (tiktokPostWebRoutePanel && isTikTokPostWebRoute(route)) tiktokPostWebRoutePanel.hidden = false;
  ...
  if (isTikTokPostWebRoute(route)) renderTikTokPostWeb();
...
function hasUsableTikTokChannelCache() {
  const selected = normalizePostAccount(showcaseSelectedAccount || postSelectedAccount);
...
const FLOW_JOBS_KEY = "autogt.flow.processing.jobs";
const tiktokPostPanel = document.getElementById("tiktokPostWebPanel");
const postClipList = document.getElementById("postClipList");
const postClipCount = document.getElementById("postClipCount");
const postClipUploadBtn = document.getElementById("postClipUploadBtn");
const postClipPickBtn = document.getElementById("postClipPickBtn");
const postClipFileInput = document.getElementById("postClipFileInput");
const postClipStatus = document.getElementById("postClipStatus");
const postPublishMode = document.getElementById("postPublishMode");
const postScheduleField = document.getElementById("postScheduleField");
const postScheduleAt = document.getElementById("postScheduleAt");
const postScheduleGap = document.getElementById("postScheduleGap");
const postProductSelect = document.getElementById("postProductSelect");
const postPublishModeButtons = document.getElementById("postPublishModeButtons");
const postAutoProduct = document.getElementById("postAutoProduct");
const postDisableCart = document.getElementById("postDisableCart");
const postAccountPickBtn = document.getElementById("postAccountPickBtn");
const postSelectedAccountCard = document.getElementById("postSelectedAccountCard");
const postCaptionTemplate = document.getElementById("postCaptionTemplate");
const postHashtags = document.getElementById("postHashtags");
const postAiLabel = document.getElementById("postAiLabel");
const postDisclose = document.getElementById("postDisclose");
const postYourBrand = document.getElementById("postYourBrand");
const postBrandedContent = document.getElementById("postBrandedContent");
const postSaveSettingsBtn = document.getElementById("postSaveSettingsBtn");
const postSettingsSavedText = document.getElementById("postSettingsSavedText");
const postAddQueueBtn = document.getElementById("postAddQueueBtn");
const postRunQueueBtn = document.getElementById("postRunQueueBtn");
const postStopQueueBtn = document.getElementById("postStopQueueBtn");
const postClearQueueBtn = document.getElementById("postClearQueueBtn");
const postRetryAllBtn = document.getElementById("postRetryAllBtn");
const postQueueTable = document.getElementById("postQueueTable");
const postQueueCount = document.getElementById("postQueueCount");
const postTotalJobs = document.getElementById("postTotalJobs");
const postDoneJobs = document.getElementById("postDoneJobs");
const postFailedJobs = document.getElementById("postFailedJobs");
const postLiveLogText = document.getElementById("postLiveLogText");
// The old Flow Queue Process panel this pointed at is gone (see the
// flow-suite port), so this now always resolves to null — kept only because
// refreshLiveLog() below still syncs it alongside the post/mobile live-log
// displays; its `if (flowLiveLogText)` guard makes the null a no-op.
const flowLiveLogText = document.getElementById("flowLiveLogText");
...
const POST_CLIPS_KEY = "autogt.tiktok.post.clips";
const POST_JOBS_KEY = "autogt.tiktok.post.jobs";
const POST_SETTINGS_KEY = "autogt.tiktok.post.settings";
const POST_ACCOUNT_CACHE_KEY = "autogt.tiktok.post.accounts";
const SHOWCASE_ACCOUNT_KEY = "autogt.tiktok.showcase.account";
const POST_SCHEDULE_MIN_LEAD_MIN = 15;
const POST_SCHEDULE_MAX_DAYS = 10;
let postClips = readPostClips();
let postJobs = readPostJobs();
let postSelectedAccount = readPostSettings().account || null;
let showcaseSelectedAccount = readShowcaseAccount();
let tiktokAccountModalMode = "post";
const postRunner = {
  running: false,
  stopRequested: false,
};
...
function flowJobsStorageKey(platform = activeFlowPlatform) {
  return `${FLOW_JOBS_KEY}.${platform}`;
}




// Read-only: still used by the TikTok post-web video picker as a fallback
// source when scanning Final-File fails, for any completed job a user still
// has sitting in old localStorage. Nothing writes new entries here anymore
// — new flow-suite jobs go straight to Final-File (see server.js).
function readFlowJobs(platform = activeFlowPlatform) {
  try {
    const rows = JSON.parse(localStorage.getItem(flowJobsStorageKey(platform)) || "[]");
...
function setActiveFlowPlatform(platform) {
  ...
  if (platformChanged) flowSuiteSamplePicks = [];
...
let mobileFloatingLogsOpen = localStorage.getItem("autogt.mobile.floatingLogs") === "1";
...
async function refreshLiveLog() {
  ...
  if (flowLiveLogText) flowLiveLogText.textContent = latest;
  if (postLiveLogText) postLiveLogText.textContent = latest;
  if (mobileLiveLogText) mobileLiveLogText.textContent = latest;
  renderMobileFloatingLogsRows(rows);
...
function readPostClips() {
  try {
    const rows = JSON.parse(localStorage.getItem(POST_CLIPS_KEY) || "[]");
    return Array.isArray(rows) ? rows : [];
  } catch {
    return [];
  }
...
function savePostClips() {
  localStorage.setItem(POST_CLIPS_KEY, JSON.stringify(postClips));
}

function readPostJobs() {
  try {
    const rows = JSON.parse(localStorage.getItem(POST_JOBS_KEY) || "[]");
    return Array.isArray(rows) ? rows : [];
  } catch {
    return [];
  }
...
function savePostJobs() {
  localStorage.setItem(POST_JOBS_KEY, JSON.stringify(postJobs));
}

function readPostSettings() {
  try {
    const settings = JSON.parse(localStorage.getItem(POST_SETTINGS_KEY) || "{}");
...
function savePostSettings() {
  const settings = getPostSettingsValues();
  localStorage.setItem(POST_SETTINGS_KEY, JSON.stringify(settings));
  return settings;
}
...
function removePostAccountCache(target) {
  ...
  if (postAccountMatches(postSelectedAccount, account)) {
    postSelectedAccount = null;
    savePostSettings();
    renderPostSelectedAccount();
  }
  return next;
}
...
function renderPostSelectedAccount() {
  if (!postSelectedAccountCard) return;
  const account = hydrateAccountFromChannelCache(postSelectedAccount);
  if (!account) {
    postSelectedAccountCard.hidden = true;
    postSelectedAccountCard.innerHTML = "";
    return;
  }
  postSelectedAccount = account;
  postSelectedAccountCard.hidden = false;
  postSelectedAccountCard.innerHTML = `
...
function openPostAccountDeleteConfirm(account, accountModal, afterDelete) {
  ...
  confirmButton.onclick = async () => {
    ...
    try {
      ...
      setPostClipStatus(`ลบโปรไฟล์ @${normalized.uniqueId} แล้ว`);
      closePostAccountDeleteModal();
    } catch (error) {
      setPostClipStatus(`ลบโปรไฟล์ไม่สำเร็จ: ${error.message || error}`, true);
    } finally {
...
function ensureTikTokAccountModal() {
    ...
    <section class="post-account-dialog" role="dialog" aria-modal="true" aria-labelledby="postAccountTitle">
      ...
      <div class="post-section-head">
        <div>
          ...
          <h2 id="postAccountTitle">เลือกบัญชีที่โพสต์</h2>
  ...
  modal.addEventListener("click", (event) => {
    ...
    if (card) {
      ...
      if (!account) return;
      if (tiktokAccountModalMode === "showcase") {
      ...
      postSelectedAccount = account;
      savePostAccountCache([account]);
      savePostSettings();
      renderPostSelectedAccount();
      setPostClipStatus(`เลือกบัญชีโพสต์ @${account.uniqueId}`);
      closeTikTokAccountModal();
    }
  });
  document.body.appendChild(modal);
...
function updateTikTokAccountModalTitle(modal) {
  ...
  if (title) title.textContent = tiktokAccountModalMode === "showcase" ? "เลือกบัญชีที่จะดึงสินค้า" : "เลือกบัญชีที่โพสต์";
  if (desc) {
    desc.textContent = tiktokAccountModalMode === "showcase"
      ? "เลือก Channel/TikTok account ที่มี cookie/token เก็บไว้ ระบบจะใช้ cache นี้เรียก API Showcase โดยตรง"
      : "เลือกบัญชีจากหน้า Channel ที่บันทึกไว้ ระบบจะใช้ cache นี้กับงานโพสต์โดยตรง";
  }
}
...
function renderTikTokAccountCards(modal, accounts = [], profiles = []) {
  ...
  list.innerHTML = accounts.map((raw) => {
    const account = normalizePostAccount(raw);
    if (!account) return "";
    const activeAccount = tiktokAccountModalMode === "showcase" ? showcaseSelectedAccount : postSelectedAccount;
    const selected = postAccountKey(activeAccount) === postAccountKey(account);
...
async function loadTikTokAccountsForPost(modal = ensureTikTokAccountModal(), options = {}) {
  ...
  if (cacheOnly) {
    ...
    // scans done from the main app's Channel tab (a different origin) — those
    // only land in the server-side cache. Fall back to that cache once so both
    // pages agree on what's been scanned, and mirror it into localStorage for
    // next time.
    if (!accounts.length) {
      try {
        ...
        const serverAccounts = Array.isArray(payload.accounts)
          ...
          : [];
        if (serverAccounts.length) accounts = savePostAccountCache(serverAccounts, { replace: false });
      } catch (_) {
        /* server unreachable — stay with the (empty) local cache */
      }
    }
...
function openTikTokAccountModal() {
  tiktokAccountModalMode = "post";
...
function setPostClipStatus(message, isError = false) {
  if (!postClipStatus) return;
  postClipStatus.textContent = message || "";
  postClipStatus.classList.toggle("is-error", !!isError);
}

function postClipKindLabel(kind) {
  return kind === "job" ? "งานที่เสร็จแล้ว" : "อัปโหลด";
}

function postStatusLabel(status, result = {}) {
  const key = String(status || "").toLowerCase();
  if (key === "publishing") return "กำลังโพสต์";
  if (key === "uploading") return "กำลังอัปโหลด";
  if (key === "done") {
    if (result.scheduled) return "ตั้งเวลาแล้ว";
    if (result.drafted) return "บันทึกร่างแล้ว";
    if (result.posted) return "โพสต์แล้ว";
    return "พร้อมตรวจโพสต์";
  }
  if (key === "failed") return "ไม่สำเร็จ";
  if (key === "stopped") return "หยุดงาน";
  return "Queued";
}

function isPostRetryable(job) {
  return ["failed", "stopped"].includes(String(job?.status || "").toLowerCase());
}

function normalizePostPublishMode(mode) {
  return ["post", "schedule"].includes(String(mode || "")) ? String(mode) : "post";
}

function applyPostSettingsValues(settings = readPostSettings()) {
  settings = { ...(settings || {}), publishMode: normalizePostPublishMode(settings?.publishMode) };
  if (postPublishMode && settings.publishMode) postPublishMode.value = settings.publishMode;
  if (postScheduleAt && settings.scheduleAt) postScheduleAt.value = settings.scheduleAt;
  if (postScheduleGap && settings.scheduleGap) postScheduleGap.value = String(settings.scheduleGap);
  if (postProductSelect && settings.productId) postProductSelect.value = settings.productId;
  if (postAutoProduct) postAutoProduct.checked = settings.autoProduct !== false;
  if (postDisableCart) postDisableCart.checked = !!settings.disableCart;
  postSelectedAccount = normalizePostAccount(settings.account) || postSelectedAccount || null;
  if (postSelectedAccount) savePostAccountCache([postSelectedAccount]);
  renderPostSelectedAccount();
  if (postCaptionTemplate && typeof settings.captionTemplate === "string") postCaptionTemplate.value = settings.captionTemplate;
  if (postHashtags && typeof settings.hashtags === "string") postHashtags.value = settings.hashtags;
  if (postAiLabel) postAiLabel.checked = !!settings.aiLabel;
  if (postDisclose) postDisclose.checked = !!settings.disclose;
  if (postYourBrand) postYourBrand.checked = !!settings.yourBrand;
  if (postBrandedContent) postBrandedContent.checked = !!settings.brandedContent;
  updatePostModeUi();
}

function getPostSettingsValues() {
  return {
    publishMode: normalizePostPublishMode(postPublishMode?.value),
    scheduleAt: postScheduleAt?.value || "",
    scheduleGap: Number(postScheduleGap?.value || 60) || 60,
    productId: postProductSelect?.value || "",
    productCta: postProductSelect?.selectedOptions?.[0]?.dataset.title || "",
    autoProduct: postAutoProduct?.checked !== false,
    disableCart: !!postDisableCart?.checked,
    account: normalizePostAccount(postSelectedAccount),
    captionTemplate: postCaptionTemplate?.value || "",
    hashtags: postHashtags?.value || "",
    aiLabel: !!postAiLabel?.checked,
    disclose: !!postDisclose?.checked,
    yourBrand: !!postYourBrand?.checked,
    brandedContent: !!postBrandedContent?.checked,
  };
}

function updatePostModeUi() {
  const schedule = postPublishMode?.value === "schedule";
  if (postScheduleField) postScheduleField.hidden = !schedule;
  postPublishModeButtons?.querySelectorAll("[data-post-mode]").forEach((button) => {
    button.classList.toggle("is-active", button.dataset.postMode === (postPublishMode?.value || "post"));
  });
  updatePostProductSwitchUi();
}

function updatePostProductSwitchUi() {
  if (postAutoProduct) postAutoProduct.disabled = false;
  if (postDisableCart) postDisableCart.disabled = false;
  [
    [postAutoProduct, "ใช้ข้อมูลสินค้าอัตโนมัติจากไฟล์ JS"],
    [postDisableCart, "ปิดการปักสินค้า/ตะกร้าในโพสต์นี้"],
  ].forEach(([input, title]) => {
    const card = input?.closest(".post-switch-card");
    if (!card) return;
...
function togglePostSwitchInput(input) {
...
function addMinutesToInputDateTime(value, minutes) {
  ...
  const next = new Date(time + minutes * 60 * 1000);
  const pad = (n) => String(n).padStart(2, "0");
  return `${next.getFullYear()}-${pad(next.getMonth() + 1)}-${pad(next.getDate())}T${pad(next.getHours())}:${pad(next.getMinutes())}`;
}

function validatePostSchedule(settings, count) {
  if (settings.publishMode !== "schedule") return true;
  const base = Date.parse(settings.scheduleAt || "");
  if (!Number.isFinite(base)) {
    setPostClipStatus("กรุณาเลือกเวลาที่จะตั้งโพสต์", true);
    return false;
  }
  const minTime = Date.now() + POST_SCHEDULE_MIN_LEAD_MIN * 60 * 1000;
  const maxTime = Date.now() + POST_SCHEDULE_MAX_DAYS * 24 * 60 * 60 * 1000;
  const tail = base + Math.max(0, count - 1) * Number(settings.scheduleGap || 60) * 60 * 1000;
  if (base < minTime) {
    setPostClipStatus(`TikTok ต้องตั้งเวลาล่วงหน้าอย่างน้อย ${POST_SCHEDULE_MIN_LEAD_MIN} นาที`, true);
    return false;
  }
  if (tail > maxTime) {
    setPostClipStatus(`โพสต์สุดท้ายเลย ${POST_SCHEDULE_MAX_DAYS} วัน ให้ลดจำนวนคลิปหรือระยะห่าง`, true);
    return false;
  }
  return true;
}
...
function tiktokProductsForPostSelect() {
  const rows = Array.isArray(showcaseState?.products) ? showcaseState.products : [];
  return rows.filter((product) => product.id || product.productId);
}

function renderPostProductOptions() {
  if (!postProductSelect) return;
  const current = postProductSelect.value;
  const products = tiktokProductsForPostSelect();
  postProductSelect.innerHTML = [
    `<option value="">ไม่ปักสินค้า</option>`,
    ...products.map((product) =>
      `<option value="${escapeHtml(product.id || product.productId || "")}" data-title="${escapeHtml(product.title || "")}">${escapeHtml(product.title || product.id || "สินค้า")}</option>`
    ),
  ].join("");
  if ([...postProductSelect.options].some((option) => option.value === current)) postProductSelect.value = current;
}

let selectedPostClipKey = "";

function renderPostClips() {
  if (!postClipList) return;
  if (selectedPostClipKey && !postClips.some((clip) => clip.key === selectedPostClipKey)) selectedPostClipKey = "";
  postClipList.innerHTML = postClips.length ? postClips.map((clip, index) => {
    const safeLabel = cleanDisplayText(clip.label, clip.productId ? productFallbackTitle(clip.productId, "TikTok") : "คลิป");
    const safeCaption = cleanDisplayText(clip.caption, "");
    const isSelected = clip.key === selectedPostClipKey;
    return `
    <article class="post-clip-card${isSelected ? " is-selected" : ""}" data-post-clip="${escapeHtml(clip.key)}">
      <span class="post-clip-index">${index + 1}</span>
      <button class="post-clip-thumb" type="button" data-media="${escapeHtml(clip.url)}" data-media-type="video" data-media-title="${escapeHtml(safeLabel)}">
        <video src="${escapeHtml(clip.url)}" muted preload="metadata" playsinline></video>
        <span class="flow-media-play"><span class="material-symbols-outlined">play_arrow</span></span>
      </button>
      <div class="post-clip-main">
        <strong title="${escapeHtml(safeLabel || "-")}">${escapeHtml(safeLabel || "-")}</strong>
        <span>${escapeHtml(postClipKindLabel(clip.kind))}</span>
        <input type="text" data-post-caption="${escapeHtml(clip.key)}" placeholder="Caption เฉพาะคลิปนี้" value="${escapeHtml(safeCaption)}" />
      </div>
      <button class="flow-remove-btn" type="button" data-post-clip-remove="${escapeHtml(clip.key)}" aria-label="ลบคลิป">
        <span class="material-symbols-outlined">close</span>
      </button>
    </article>
  `;
  }).join("") : `<div class="flow-empty-card">ยังไม่มีคลิป กดอัปโหลดคลิป หรือเลือกจากงานที่เสร็จแล้ว</div>`;
  if (postClipCount) postClipCount.textContent = `${postClips.length} คลิป`;
  if (postAddQueueBtn) postAddQueueBtn.disabled = postClips.length === 0;
  postClipList.querySelectorAll("[data-post-caption]").forEach((field) => {
    field.addEventListener("input", () => {
      postClips = postClips.map((clip) => clip.key === field.dataset.postCaption ? { ...clip, caption: field.value } : clip);
      savePostClips();
    });
  });
  postClipList.querySelectorAll("[data-post-clip-remove]").forEach((button) => {
    button.addEventListener("click", () => {
      postClips = postClips.filter((clip) => clip.key !== button.dataset.postClipRemove);
      savePostClips();
      renderPostClips();
    });
  });
  postClipList.querySelectorAll(".post-clip-card").forEach((card) => {
    card.addEventListener("click", (event) => {
      if (event.target.closest("button, input, textarea, select, a")) return;
      const key = card.dataset.postClip;
      selectedPostClipKey = selectedPostClipKey === key ? "" : key;
      renderPostClips();
    });
  });
}

function isTikTokFinalLibraryVideo(video) {
  const source = String(video?.source || "").toLowerCase();
  const platform = String(video?.platform || "").toLowerCase();
  const pathish = `${video?.path || ""} ${video?.url || ""}`.toLowerCase().replace(/\\/g, "/");
  return source === "final-file" && (platform === "tiktok" || pathish.includes("/final-file/tiktok/"));
}

async function allCompletedVideoJobs() {
  try {
    setPostClipStatus("กำลังอ่านคลิปจาก Final-File/TikTok...");
    const response = await fetch("/api/mobile/library/scan", { cache: "no-store" });
    const payload = await readJsonResponse(response);
    if (!response.ok || payload.ok === false) throw new Error(payload.error || "Scan Final TikTok failed");
    const videos = Array.isArray(payload.videos) ? payload.videos : [];
    const finalTikTok = videos
      .filter(isTikTokFinalLibraryVideo)
      .map((video) => {
        const productId = video.productId || video.productID || "";
        const fallback = productFallbackTitle(productId, "TikTok");
        const titleText = cleanDisplayText(video.productName || video.title || video.name || "", fallback);
        const captionText = cleanDisplayText(video.caption || video.productName || video.title || "", titleText);
        return {
          id: video.id || video.name || video.url,
          mediaUrl: video.url || "",
          thumb: "",
          title: titleText,
          productId,
          productCta: titleText,
          caption: captionText,
          source: video.source || "Final-File",
          path: video.path || "",
          mtime: video.mtime || "",
        };
      })
      .filter((video) => video.mediaUrl);
    if (finalTikTok.length) return finalTikTok;
  } catch (error) {
    sendUiLog("warn", "tiktok-post:ui", "Final TikTok scan failed; falling back to local completed jobs", {
  ...
  return sources
    ...
    .map((job) => {
      const productId = job.productId || job.productID || "";
      const titleText = cleanDisplayText(job.title || job.productName || "", productFallbackTitle(productId, "TikTok"));
      return {
        ...job,
        title: titleText,
        mediaUrl: flowJobResultUrl(job),
        thumb: flowJobVideoThumbUrl(job),
        productId,
        productCta: titleText,
        caption: cleanDisplayText(job.caption || job.title || "", titleText),
      };
    })
    .filter((job) => job.mediaUrl)
    .filter((job) => {
      const key = job.id || job.mediaUrl;
...
function ensurePostClipPickerModal() {
  let modal = document.getElementById("postClipPickerModal");
  if (modal) return modal;
  modal = document.createElement("div");
  modal.id = "postClipPickerModal";
  modal.className = "post-picker-modal";
  modal.innerHTML = `
    <div class="post-picker-backdrop" data-post-picker-close></div>
    <section class="post-picker-dialog" role="dialog" aria-modal="true" aria-labelledby="postPickerTitle">
      <button class="flow-prompt-close" type="button" data-post-picker-close aria-label="Close">
        <span class="material-symbols-outlined">close</span>
      </button>
      <div class="post-section-head">
        <div>
          <p class="eyebrow">Completed videos</p>
          <h2 id="postPickerTitle">เลือกคลิปที่สร้างเสร็จแล้ว</h2>
        </div>
        <div class="post-head-actions post-picker-actions">
          <button class="outline-btn compact" type="button" data-post-picker-select-all>เลือกทั้งหมด</button>
          <button class="primary-btn compact" type="button" data-post-picker-add disabled>เพิ่ม</button>
        </div>
      </div>
      <div class="post-picker-grid" data-post-picker-grid></div>
    </section>
  `;
  modal.addEventListener("click", (event) => {
    if (event.target.closest("[data-post-picker-close]")) {
      closePostClipPickerModal();
      return;
    }
    if (event.target.closest("[data-post-picker-select-all]")) {
      toggleAllPostPickerCards(modal);
      return;
    }
    if (event.target.closest("[data-post-picker-add]")) {
      addPickedCompletedClips();
      return;
    }
    const card = event.target.closest("[data-post-pick-id]");
    if (card) {
      card.classList.toggle("is-selected");
      updatePostPickerActions(modal);
    }
  });
  document.body.appendChild(modal);
  return modal;
...
function updatePostPickerActions(modal) {
  const cards = [...(modal?.querySelectorAll(".post-picker-card") || [])];
  const selected = cards.filter((card) => card.classList.contains("is-selected"));
  const add = modal?.querySelector("[data-post-picker-add]");
  const selectAll = modal?.querySelector("[data-post-picker-select-all]");
  if (add) add.disabled = selected.length === 0;
  if (selectAll) {
    selectAll.disabled = cards.length === 0;
    selectAll.textContent = cards.length > 0 && selected.length === cards.length ? "ยกเลิกเลือกทั้งหมด" : "เลือกทั้งหมด";
  }
}

function toggleAllPostPickerCards(modal) {
  const cards = [...(modal?.querySelectorAll(".post-picker-card") || [])];
  const shouldSelect = cards.some((card) => !card.classList.contains("is-selected"));
  cards.forEach((card) => card.classList.toggle("is-selected", shouldSelect));
  updatePostPickerActions(modal);
}

async function openPostClipPickerModal() {
  const jobs = await allCompletedVideoJobs();
  if (!jobs.length) {
    setPostClipStatus("ยังไม่มีคลิปใน Final-File/TikTok ให้เลือก", true);
    return;
  }
  const modal = ensurePostClipPickerModal();
  const grid = modal.querySelector("[data-post-picker-grid]");
  if (grid) {
    grid.innerHTML = jobs.map((job) => `
      <article class="post-picker-card" data-post-pick-id="${escapeHtml(job.id || "")}" data-post-pick-url="${escapeHtml(job.mediaUrl)}" data-post-pick-title="${escapeHtml(job.title || job.id || "คลิป")}" data-post-pick-product-id="${escapeHtml(job.productId || "")}" data-post-pick-product-cta="${escapeHtml(job.productCta || "")}" data-post-pick-caption="${escapeHtml(job.caption || "")}">
        <span class="post-picker-check"><span class="material-symbols-outlined">check</span></span>
        <button class="post-picker-thumb" type="button" aria-label="เลือกคลิป">
          ${job.thumb ? `<img src="${escapeHtml(job.thumb)}" alt="" loading="lazy" />` : `<video src="${escapeHtml(job.mediaUrl)}" muted preload="metadata" playsinline></video>`}
        </button>
        <strong title="${escapeHtml(job.title || "-")}">${escapeHtml(job.title || "-")}</strong>
        <span>${escapeHtml(job.productId ? `Product ID: ${job.productId}` : "Final TikTok")}</span>
      </article>
    `).join("");
  }
  updatePostPickerActions(modal);
...
function closePostClipPickerModal() {
  const modal = document.getElementById("postClipPickerModal");
  if (!modal) return;
  modal.classList.remove("is-open");
  document.body.classList.remove("post-picker-open");
}

function addPickedCompletedClips() {
  const modal = document.getElementById("postClipPickerModal");
  if (!modal) return;
  const cards = [...modal.querySelectorAll(".post-picker-card.is-selected")];
  if (!cards.length) return;
  for (const card of cards) {
    const productId = card.dataset.postPickProductId || "";
    const label = cleanDisplayText(card.dataset.postPickTitle || "", productFallbackTitle(productId, "TikTok"));
    const caption = cleanDisplayText(card.dataset.postPickCaption || "", label);
    postClips.push({
      key: `job-${Date.now()}-${Math.random().toString(16).slice(2)}`,
      kind: "job",
      jobId: card.dataset.postPickId || "",
      url: card.dataset.postPickUrl || "",
      label,
      productId,
      productCta: cleanDisplayText(card.dataset.postPickProductCta || "", label),
      caption,
      createdAt: new Date().toISOString(),
    });
  }
  ...
  renderPostClips();
  closePostClipPickerModal();
  setPostClipStatus(`เพิ่มคลิปแล้ว ${cards.length} รายการ`);
}

async function uploadPostClip(file) {
  if (!file) return;
  setPostClipStatus(`กำลังอัปโหลด ${file.name}...`);
  const form = new FormData();
  form.append("file", file);
  const response = await fetch("/api/upload-clip", { method: "POST", body: form, cache: "no-store" });
  const payload = await readJsonResponse(response);
  if (!response.ok || payload.ok === false || !payload.url) throw new Error(payload.error || "Upload failed");
  postClips.push({
    key: `upload-${Date.now()}-${Math.random().toString(16).slice(2)}`,
    kind: "upload",
    url: payload.url,
    label: file.name,
    caption: "",
...
async function pickPostClipFiles() {
  if (postClipUploadBtn?.disabled) return;
  if (typeof window.showOpenFilePicker === "function") {
    try {
      ...
      if (files.length) await uploadPostClips(files);
      return;
    } catch (error) {
      if (error?.name === "AbortError") return;
      sendUiLog("warn", "tiktok-post:ui", "Native file picker failed, fallback to input", { error: String(error.message || error) });
    }
  }
  if (!postClipFileInput) {
    setPostClipStatus("ไม่พบตัวเลือกไฟล์สำหรับอัปโหลด", true);
    return;
  }
  postClipFileInput.value = "";
  postClipFileInput.click();
}
async function uploadPostClips(files) {
  const list = [...(files || [])].filter((file) => /^video\//i.test(file.type || "") || /\.(mp4|webm|mov)$/i.test(file.name || ""));
  if (!list.length) {
    setPostClipStatus("กรุณาเลือกไฟล์วิดีโอ", true);
    return;
  }
  if (postClipUploadBtn) postClipUploadBtn.disabled = true;
  try {
    for (const file of list) await uploadPostClip(file);
    setPostClipStatus(`อัปโหลดเสร็จ ${list.length} คลิป`);
  } catch (error) {
    setPostClipStatus(`อัปโหลดไม่สำเร็จ: ${error.message || error}`, true);
  } finally {
    if (postClipUploadBtn) postClipUploadBtn.disabled = false;
    if (postClipFileInput) postClipFileInput.value = "";
  }
}

function postFinalizeFromMode(mode) {
  return normalizePostPublishMode(mode) === "schedule" ? "schedule" : "post";
}

function postAbsoluteMediaUrl(url) {
...
function queuePostClips() {
  if (!postClips.length) {
    setPostClipStatus("เพิ่มคลิปอย่างน้อย 1 คลิปก่อน", true);
    return;
  }
  const settings = savePostSettings();
  if (!validatePostSchedule(settings, postClips.length)) return;
  const hashtags = parsePostHashtags(settings.hashtags);
  const jobs = postClips.map((clip, index) => {
    const scheduleAt = settings.publishMode === "schedule"
      ? addMinutesToInputDateTime(settings.scheduleAt, Number(settings.scheduleGap || 60) * index)
      : "";
    const productId = settings.disableCart
      ? ""
      : (settings.autoProduct ? (clip.productId || settings.productId || "") : (settings.productId || ""));
    const productCta = settings.disableCart
      ? ""
      : (settings.autoProduct ? (clip.productCta || clip.label || settings.productCta || "") : (settings.productCta || ""));
    return {
      id: `#TP-${Date.now().toString().slice(-5)}-${String(index + 1).padStart(2, "0")}`,
      clipKey: clip.key,
      mediaUrl: clip.url,
      label: clip.label || "คลิป TikTok",
      caption: (clip.caption || settings.captionTemplate || "").trim(),
      hashtags,
      productId,
      productCta,
      account: normalizePostAccount(settings.account),
      publishMode: settings.publishMode,
      finalize: postFinalizeFromMode(settings.publishMode),
      scheduleAt,
      settings: {
        autoProduct: settings.autoProduct,
        disableCart: settings.disableCart,
        aiLabel: settings.aiLabel,
        disclose: settings.disclose,
  ...
  postJobs = [...postJobs, ...jobs];
  postClips = [];
  savePostJobs();
  savePostClips();
  renderTikTokPostWeb();
  setPostClipStatus(`เพิ่มลง Queue แล้ว ${jobs.length} งาน`);
}

function updatePostRunnerButtons() {
  const hasJobs = postJobs.length > 0;
  const hasRetry = postJobs.some(isPostRetryable);
  if (postRunQueueBtn) postRunQueueBtn.disabled = postRunner.running || !postJobs.some((job) => ["queued", "failed", "stopped"].includes(String(job.status || "").toLowerCase()));
  if (postStopQueueBtn) postStopQueueBtn.disabled = !postRunner.running;
  if (postClearQueueBtn) postClearQueueBtn.disabled = postRunner.running || !hasJobs;
  if (postRetryAllBtn) postRetryAllBtn.disabled = postRunner.running || !hasRetry;
}

function updatePostStats() {
  const done = postJobs.filter((job) => String(job.status || "").toLowerCase() === "done").length;
  const failed = postJobs.filter(isPostRetryable).length;
  if (postTotalJobs) postTotalJobs.textContent = String(postJobs.length);
  if (postDoneJobs) postDoneJobs.textContent = String(done);
  if (postFailedJobs) postFailedJobs.textContent = String(failed);
  if (postQueueCount) postQueueCount.textContent = `${postJobs.length} งาน`;
}

function renderPostJobs() {
  if (!postQueueTable) return;
  if (!postJobs.length) {
    postQueueTable.innerHTML = `<div class="flow-empty-card">ยังไม่มี Queue Post เพิ่มคลิปลง Queue ก่อน</div>`;
    updatePostStats();
    updatePostRunnerButtons();
    return;
  }
  postQueueTable.innerHTML = `
    <div class="post-job-row is-head">
      <span>Job-ID</span>
      <span>คลิป</span>
      <span>Caption</span>
      <span>วิธีลง</span>
    ...
    ${postJobs.map((job) => `
      <div class="post-job-row" data-post-status="${escapeHtml(String(job.status || "queued").toLowerCase())}">
        <span class="flow-job-id">${escapeHtml(job.id || "")}</span>
        <button class="post-job-thumb" type="button" data-media="${escapeHtml(job.mediaUrl || "")}" data-media-type="video" data-media-title="${escapeHtml(job.label || "คลิป")}">
          <video src="${escapeHtml(job.mediaUrl || "")}" muted preload="metadata" playsinline></video>
          <span class="flow-media-play"><span class="material-symbols-outlined">play_arrow</span></span>
        </button>
        <strong title="${escapeHtml(job.caption || job.label || "-")}">${escapeHtml(job.caption || job.label || "-")}</strong>
        <span>${escapeHtml(postPublishModeText(job.publishMode, job.scheduleAt))}</span>
        <span class="flow-status-wrap">
          <span class="flow-status" title="${escapeHtml(job.error || job.result?.note || "")}">${escapeHtml(postStatusLabel(job.status, job.result || {}))}</span>
          ${isPostRetryable(job) ? `<button class="flow-retry-btn" type="button" data-post-job-retry="${escapeHtml(job.id || "")}"><span class="material-symbols-outlined">restart_alt</span> Retry</button>` : ""}
        </span>
        <button class="flow-remove-btn" type="button" data-post-job-remove="${escapeHtml(job.id || "")}" aria-label="ลบงาน">
          <span class="material-symbols-outlined">close</span>
        </button>
      </div>
    `).join("")}
  `;
  postQueueTable.querySelectorAll("[data-post-job-remove]").forEach((button) => {
    button.addEventListener("click", () => {
      postJobs = postJobs.filter((job) => job.id !== button.dataset.postJobRemove);
      savePostJobs();
      renderPostJobs();
    });
  });
  postQueueTable.querySelectorAll("[data-post-job-retry]").forEach((button) => {
    button.addEventListener("click", () => retryPostJob(button.dataset.postJobRetry));
  });
  updatePostStats();
  updatePostRunnerButtons();
}

function postPublishModeText(mode, scheduleAt = "") {
  return normalizePostPublishMode(mode) === "schedule"
    ? (scheduleAt ? `ตั้งเวลา ${scheduleAt.replace("T", " ")}` : "ตั้งเวลา")
    : "โพสต์เลย";
}

function setPostJob(job, patch) {
...
async function publishPostJob(job) {
  setPostJob(job, { status: "publishing", error: "" });
  const mediaUrl = postAbsoluteMediaUrl(job.mediaUrl);
  sendUiLog("info", "tiktok-post:ui", "Sending TikTok publish job", {
    endpoint: "/api/tiktok/post-web/publish",
    jobId: job.id,
    mediaUrl,
    publishMode: job.publishMode,
    scheduleAt: job.scheduleAt || "",
    productId: job.productId || "",
    captionPreview: shortLogText(job.caption || "", 180),
  });
  const response = await fetch("/api/tiktok/post-web/publish", {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({
      jobId: job.id,
      ...
      account: normalizePostAccount(job.account),
      finalize: job.finalize || postFinalizeFromMode(job.publishMode),
      scheduleAt: job.scheduleAt || "",
      autoPost: (job.finalize || postFinalizeFromMode(job.publishMode)) === "post",
      settings: job.settings || {},
    }),
    cache: "no-store",
  });
  const payload = await readJsonResponse(response);
  if (!response.ok || payload.ok === false) throw new Error(payload.error || payload.note || "TikTok publish failed");
  return payload;
}

async function runPostQueue() {
  if (postRunner.running) return;
  postRunner.running = true;
  postRunner.stopRequested = false;
  updatePostRunnerButtons();
  try {
    for (const job of postJobs) {
      if (postRunner.stopRequested) break;
      const status = String(job.status || "").toLowerCase();
      if (!["queued", "failed", "stopped"].includes(status)) continue;
      try {
        const result = await publishPostJob(job);
        setPostJob(job, {
          status: "done",
          result,
          error: "",
        ...
        setPostJob(job, {
          status: postRunner.stopRequested ? "stopped" : "failed",
          error: String(error.message || error),
        });
        sendUiLog(postRunner.stopRequested ? "warn" : "error", "tiktok-post:ui", postRunner.stopRequested ? "TikTok post stopped" : "TikTok post failed", {
          jobId: job.id,
          message: String(error.message || error),
        });
        if (!postRunner.stopRequested) break;
    ...
    postRunner.running = false;
    updatePostRunnerButtons();
    renderPostJobs();
  }
}

function retryPostJob(jobId) {
  if (postRunner.running) return;
  postJobs = postJobs.map((job) => job.id === jobId ? { ...job, status: "queued", error: "", updatedAt: new Date().toISOString() } : job);
  savePostJobs();
  renderPostJobs();
  runPostQueue();
}

function retryAllPostJobs() {
  if (postRunner.running) return;
  postJobs = postJobs.map((job) => isPostRetryable(job) ? { ...job, status: "queued", error: "", updatedAt: new Date().toISOString() } : job);
  savePostJobs();
  renderPostJobs();
  runPostQueue();
}

function renderTikTokPostWeb() {
  if (!tiktokPostPanel) return;
  renderPostProductOptions();
  applyPostSettingsValues();
  renderPostClips();
  renderPostJobs();
  if (postLiveLogText && flowLiveLogText) postLiveLogText.textContent = flowLiveLogText.textContent || "ยังไม่มี log ล่าสุด";
}
...
function ensureMobileFloatingLogs() {
  let panel = document.getElementById("mobileFloatingLogs");
  if (panel) return panel;
  panel = document.createElement("section");
  panel.id = "mobileFloatingLogs";
  panel.className = "mobile-floating-logs";
  panel.innerHTML = `
    <div class="mobile-floating-log-head">
      <div>
        <span class="logs-kicker">Floating trace</span>
        <strong>Activity Log</strong>
      </div>
      <div class="mobile-floating-log-actions">
        <button class="outline-btn compact" type="button" data-mobile-refresh-float-logs>
          <span class="material-symbols-outlined">refresh</span>
          Refresh
        </button>
        <button class="outline-btn compact danger" type="button" data-mobile-clear-float-logs>
          <span class="material-symbols-outlined">delete_sweep</span>
          Clear
        </button>
        <button class="outline-btn compact" type="button" data-mobile-open-modal-logs>
          <span class="material-symbols-outlined">open_in_full</span>
          Modal
        </button>
        <button class="icon-btn compact" type="button" data-mobile-close-float-logs aria-label="Close floating logs">
          <span class="material-symbols-outlined">expand_more</span>
        </button>
      </div>
    </div>
    <div class="mobile-floating-log-list" data-mobile-floating-log-list></div>
  `;
  panel.addEventListener("click", (event) => {
    if (event.target.closest("[data-mobile-close-float-logs]")) toggleMobileFloatingLogs(false);
    if (event.target.closest("[data-mobile-refresh-float-logs]")) refreshLiveLog();
    if (event.target.closest("[data-mobile-clear-float-logs]")) clearBackendLogs();
    if (event.target.closest("[data-mobile-open-modal-logs]")) openLogsModal();
  });
  document.body.appendChild(panel);
  return panel;
}

function renderMobileFloatingLogsRows(rows = backendLogRows) {
  const panel = ensureMobileFloatingLogs();
  panel.classList.toggle("is-open", mobileFloatingLogsOpen);
  document.body.classList.toggle("mobile-floating-logs-open", mobileFloatingLogsOpen);
  const list = panel.querySelector("[data-mobile-floating-log-list]");
  if (!list) return;
  const visibleRows = rows.slice(0, 80);
  list.innerHTML = visibleRows.length ? visibleRows.map((row) => {
    const time = row.time ? new Date(row.time).toLocaleTimeString("th-TH", { hour: "2-digit", minute: "2-digit", second: "2-digit" }) : "-";
    const detail = shortLogText(logDetailText(row.detail), 220);
    return `
      <article class="mobile-floating-log-row ${logLevelClass(row.level)}">
        <time>${escapeHtml(time)}</time>
        <strong>[${escapeHtml(row.level || "info")}] [${escapeHtml(row.source || "server")}] ${escapeHtml(row.message || "")}</strong>
        ${detail ? `<span>${escapeHtml(detail)}</span>` : ""}
      </article>`;
  }).join("") : '<div class="mobile-empty">No backend log yet.</div>';
}

function toggleMobileFloatingLogs(open = !mobileFloatingLogsOpen) {
  mobileFloatingLogsOpen = Boolean(open);
  localStorage.setItem("autogt.mobile.floatingLogs", mobileFloatingLogsOpen ? "1" : "0");
  renderMobileFloatingLogsRows(backendLogRows);
}
...
// update the Flow Process stats in the UI

// Helper functions to extract URLs from flow job objects, prioritizing generated images, product images, and video thumbnails.
function flowJobGeneratedImageUrl(job) {
  return job.generatedImage
    || job.generatedImageUrl
    || job.imageResultUrl
    || job.imageOutputUrl
    || job.generatedImagePath
    || job.imageRaw?.localUrl
    || job.imageRaw?.saved?.url
    || job.imageRaw?.mediaUrl
    || job.imageRaw?.url
    || "";
}

function flowJobVideoThumbUrl(job) {
  return job.videoThumbnailUrl
    || job.thumbnailUrl
    || job.videoRaw?.thumbnailUrl
    || job.videoRaw?.thumbUrl
    || job.videoRaw?.posterUrl
    || flowJobGeneratedImageUrl(job)
    || job.productImage
    || "";
}
function flowJobResultUrl(job) {
  return job.finalUrl
    || job.finalVideoUrl
    || job.finalRaw?.resultUrl
    || job.finalRaw?.url
    || job.videoRaw?.mediaUrl
    || job.videoRaw?.raw?.mediaUrl
    || job.raw?.mediaUrl
    || job.mediaUrl
    || job.remoteUrl
    || job.videoRaw?.url
    ...
    || job.videoUrl
    || job.outputUrl
    || job.videoRaw?.localUrl
    || job.videoRaw?.saved?.url
    || job.resultPath
    || job.outputPath
    || "";
}
...
function renderFlowSuiteSampleBox() {
  ...
  box.innerHTML = flowSuiteSamplePicks.map((product, index) => {
    ...
    return `
      <div class="fs-sample-card" data-sample-key="${escapeHtml(key)}">
        ...
        ${values.textMode === "withText" ? `<button type="button" class="fs-btn fs-ghost fs-test-image-btn" data-test-image="${escapeHtml(key)}">\ud83e\uddea \u0e17\u0e14\u0e2a\u0e2d\u0e1a\u0e2a\u0e23\u0e49\u0e32\u0e07\u0e23\u0e39\u0e1b</button>` : ""}
  ...
  box.querySelectorAll("[data-test-image]").forEach((button) => {
    button.addEventListener("click", async () => {
      ...
      const values = getFlowSuiteFormValues();
      const statusEl = box.querySelector(`[data-test-status="${CSS.escape(key)}"]`);
      button.disabled = true;
      if (statusEl) statusEl.textContent = "\u0e01\u0e33\u0e25\u0e31\u0e07\u0e2a\u0e23\u0e49\u0e32\u0e07\u0e23\u0e39\u0e1b\u2026";
      try {
        ...
        flowSuiteTestImages[key] = { url: payload.url, textMode: values.textMode };
      } catch (error) {
        flowSuiteTestImages[key] = { error: String(error.message || error), textMode: values.textMode };
      } finally {
        button.disabled = false;
        renderFlowSuiteSampleBox();
      }
...
function renderFlowSuiteQueue() {
  ...
  if (counts) counts.textContent = `\u0e17\u0e31\u0e49\u0e07\u0e2b\u0e21\u0e14 ${jobs.length} \u00b7 \u0e23\u0e2d\u0e04\u0e34\u0e27 ${byStatus.queued} \u00b7 \u0e01\u0e33\u0e25\u0e31\u0e07\u0e17\u0e33 ${byStatus.running} \u00b7 \u0e25\u0e49\u0e21\u0e40\u0e2b\u0e25\u0e27 ${byStatus.failed}`;
  list.innerHTML = jobs.length
    ? jobs.map((job) => renderFlowSuiteJobRow(job)).join("")
...
function maybeAutoRunFlowSuiteQueue() {
...
postPublishMode?.addEventListener("change", () => {
  updatePostModeUi();
  savePostSettings();
});
...
[
  ...
  field?.addEventListener("change", () => {
    savePostSettings();
    if (field === postAutoProduct || field === postDisableCart) updatePostProductSwitchUi();
  });
  field?.addEventListener("input", () => {
    savePostSettings();
    if (field === postAutoProduct || field === postDisableCart) updatePostProductSwitchUi();
...
postPublishModeButtons?.querySelectorAll("[data-post-mode]").forEach((button) => {
  button.addEventListener("click", () => {
    if (postPublishMode) postPublishMode.value = button.dataset.postMode || "post";
    savePostSettings();
    updatePostModeUi();
  });
});

[postAutoProduct, postDisableCart].forEach((input) => {
  const card = input?.closest(".post-switch-card");
  if (!card) return;
  card.addEventListener("click", (event) => {
    event.preventDefault();
    togglePostSwitchInput(input);
  });
...
postDisableCart?.addEventListener("change", () => {
  savePostSettings();
  updatePostModeUi();
});

postAccountPickBtn?.addEventListener("click", openTikTokAccountModal);
showcaseAccountPickBtn?.addEventListener("click", openShowcaseAccountModal);

document.querySelectorAll(".post-settings-tabs [data-post-settings-tab]").forEach((tabButton) => {
  tabButton.addEventListener("click", () => {
    document.querySelectorAll(".post-settings-tabs [data-post-settings-tab]").forEach((btn) => btn.classList.remove("is-active"));
    tabButton.classList.add("is-active");
    document.querySelectorAll(".post-settings-tab-pane").forEach((pane) => {
      pane.hidden = pane.dataset.tabPane !== tabButton.dataset.postSettingsTab;
    });
  });
});

postSaveSettingsBtn?.addEventListener("click", () => {
  savePostSettings();
  if (postSettingsSavedText) {
    postSettingsSavedText.textContent = "บันทึกแล้ว";
    window.setTimeout(() => { postSettingsSavedText.textContent = ""; }, 1800);
  }
});

postClipUploadBtn?.addEventListener("click", () => {
  pickPostClipFiles();
});

postClipFileInput?.addEventListener("change", () => {
  uploadPostClips(postClipFileInput.files);
});

postClipList?.addEventListener("dragover", (event) => {
  event.preventDefault();
  postClipList.classList.add("is-dragging");
});

postClipList?.addEventListener("dragleave", () => {
  postClipList.classList.remove("is-dragging");
});

postClipList?.addEventListener("drop", (event) => {
  event.preventDefault();
  postClipList.classList.remove("is-dragging");
  uploadPostClips(event.dataTransfer?.files || []);
});

postClipPickBtn?.addEventListener("click", () => {
  openPostClipPickerModal().catch((error) => {
    setPostClipStatus(`เปิดรายการ Final TikTok ไม่สำเร็จ: ${error.message || error}`, true);
  });
});
postAddQueueBtn?.addEventListener("click", queuePostClips);
postRunQueueBtn?.addEventListener("click", runPostQueue);
postStopQueueBtn?.addEventListener("click", () => {
  postRunner.stopRequested = true;
  updatePostRunnerButtons();
  sendUiLog("warn", "tiktok-post:ui", "Stop requested for TikTok post queue");
});
postClearQueueBtn?.addEventListener("click", () => {
  if (postRunner.running) return;
  postJobs = [];
  savePostJobs();
  renderPostJobs();
});
postRetryAllBtn?.addEventListener("click", retryAllPostJobs);
...
mobileToggleFloatLogsBtn?.addEventListener("click", () => toggleMobileFloatingLogs());

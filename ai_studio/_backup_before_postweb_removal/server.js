...
let mainExtensionWakePromise = null;
...
function queueReload(file) {
  ...
  if (file.includes("runtime") || file.endsWith(".log")) return;
  const normalized = String(file).replace(/\\/g, "/");
  if (normalized.includes("lib/media-core/flow-results")) return;
...
function tiktokProfilesResultHasAccount(result, account = {}) {
  const accounts = Array.isArray(result?.accounts) ? result.accounts : [];
  const wantedUniqueId = normalizeTikTokUniqueId(account.uniqueId || account.username);
  const wantedSecUid = String(account.secUid || "").trim();
  return accounts.some((candidate) => {
    const uniqueId = normalizeTikTokUniqueId(candidate.uniqueId || candidate.username);
    const secUid = String(candidate.secUid || "").trim();
    return (wantedSecUid && secUid && wantedSecUid === secUid) ||
      (wantedUniqueId && uniqueId && wantedUniqueId === uniqueId);
  });
}

async function verifyTikTokAccountSocket(socket, account = {}, reason = "selected") {
  if (!socket) return false;
  if (!account?.uniqueId && !account?.secUid) return true;
  const meta = extensionSocketMeta.get(socket) || {};
  const hint = chromeProfileHintsBySocketId.get(Number(meta.id)) || {};
  const wantedInstallId = String(account.installId || "").trim();
  const wantedStableId = chromeProfileStableId(account);
  const wantedUserDataDir = String(account.userDataDir || "").trim();
  const wantedProfileDirectory = String(account.profileDirectory || account.chromeProfileDirectory || "").trim();
  const socketStableId = chromeProfileStableId(hint);
  if (
    (wantedInstallId && wantedInstallId === String(meta.installId || "").trim()) ||
    (wantedStableId && socketStableId && wantedStableId === socketStableId) ||
    (wantedUserDataDir && wantedProfileDirectory &&
      wantedUserDataDir === String(hint.userDataDir || "").trim() &&
      wantedProfileDirectory === String(hint.profileDirectory || "").trim())
  ) {
    addBackendLog("info", "tiktok-post", "Selected TikTok account matched saved Chrome profile", {
      reason,
      uniqueId: account.uniqueId || "",
      socketId: meta.id || null,
      installId: meta.installId || "",
      profileLabel: meta.profileLabel || hint.profileLabel || "",
    });
    return true;
  }
  try {
    const result = await sendExtensionCommand("getTikTokProfiles", { cleanupAfterScan: true }, 60000, socket, EXTENSION_ROLE_TIKTOK);
    const matched = tiktokProfilesResultHasAccount(result, account);
    addBackendLog(matched ? "info" : "warn", "tiktok-post", matched ? "Verified selected TikTok account profile" : "Selected Chrome profile does not match TikTok account", {
      reason,
      uniqueId: account.uniqueId || "",
      socketId: meta.id || null,
      installId: meta.installId || "",
    ...
    addBackendLog("warn", "tiktok-post", "Cannot verify selected TikTok account profile", {
      reason,
      uniqueId: account.uniqueId || "",
      socketId: meta.id || null,
      installId: meta.installId || "",
...
async function findTikTokAccountSocketByIdentity(account = {}) {
  if (!account || (!account.uniqueId && !account.secUid)) return null;
  for (const socket of extensionSocketCandidates(EXTENSION_ROLE_TIKTOK, { fallback: true }).reverse()) {
    const meta = extensionSocketMeta.get(socket) || {};
    try {
      const result = await sendExtensionCommand("getTikTokProfiles", { cleanupAfterScan: true }, 60000, socket, EXTENSION_ROLE_TIKTOK);
      if (tiktokProfilesResultHasAccount(result, account)) {
        addBackendLog("info", "tiktok-post", "Matched selected TikTok account to Chrome profile", {
          uniqueId: account.uniqueId || "",
          socketId: meta.id || null,
          installId: meta.installId || "",
          profileLabel: meta.profileLabel || "",
      ...
      addBackendLog("warn", "tiktok-post", "Cannot inspect TikTok account in Chrome profile", {
        socketId: meta.id || null,
        installId: meta.installId || "",
        error: String(error.message || error),
      });
...
async function resolveTikTokPostSocketForAccount(account = {}) {
  ...
  if (installId) {
    const live = extensionSocketByInstallId(installId, EXTENSION_ROLE_TIKTOK);
    if (live && await verifyTikTokAccountSocket(live, account, "installId")) return live;
  }

  if (socketId) {
    const live = extensionSocketById(socketId, EXTENSION_ROLE_TIKTOK);
    if (live && await verifyTikTokAccountSocket(live, account, "socketId")) return live;
  }

  const matched = await findTikTokAccountSocketByIdentity(account);
  if (matched) return matched;

  addBackendLog("warn", "tiktok-post", "Selected TikTok account is not connected; skip opening every Chrome profile", {
    uniqueId: account.uniqueId || "",
    installId: account.installId || "",
    chromeProfileLabel: account.chromeProfileLabel || "",
  });
...
function getFlowSuiteRunner() {
  if (!flowSuiteRunnerInstance) {
    flowSuiteRunnerInstance = createFlowSuiteRunner({
      generateFlowMedia: runFlowGenerate,
...
const server = http.createServer((req, res) => {
  ...
  if (requestUrl.pathname === "/api/tiktok/post-web/publish" && req.method === "POST") {
    addBackendLog("info", "api:tiktok-post", "POST /api/tiktok/post-web/publish");
    readJsonBody(req, 2 * 1024 * 1024)
      .then(async (payload) => {
        if (!payload.mediaUrl) throw new Error("mediaUrl is required.");
        const accountSocketId = payload.account?.socketId || payload.account?.profileSocketId || "";
        const rawAccountChromeProfileId = String(payload.account?.chromeProfileId || "").trim();
        const accountStableId = chromeProfileStableId(payload.account || {}) || (/^chrome-[a-f0-9]+$/i.test(rawAccountChromeProfileId) ? rawAccountChromeProfileId : "");
        const accountInstallId = String(payload.account?.installId || (/^\d+$/.test(rawAccountChromeProfileId) || /^chrome-[a-f0-9]+$/i.test(rawAccountChromeProfileId) ? "" : rawAccountChromeProfileId)).trim();
        const command = {
          jobId: payload.jobId || `post-${Date.now()}`,
          mediaUrl: payload.mediaUrl,
          caption: payload.caption || "",
          hashtags: Array.isArray(payload.hashtags) ? payload.hashtags : [],
          productId: payload.productId || null,
          productCta: payload.productCta || "",
          account: payload.account || null,
          finalize: payload.finalize || "post",
          scheduleAt: payload.scheduleAt || "",
          autoPost: !!payload.autoPost,
          settings: payload.settings || {},
        };
        addBackendLog("info", "tiktok-post", "Sending clip to TikTok Web extension", {
          jobId: command.jobId,
          ...
          account: command.account ? {
            uniqueId: command.account.uniqueId || "",
            nickname: command.account.nickname || "",
          } : null,
          captionPreview: String(command.caption || "").slice(0, 220),
          hashtags: command.hashtags,
          socketId: accountSocketId || "",
          installId: accountInstallId || "",
          chromeProfileStableId: accountStableId || "",
          chromeProfileLabel: command.account?.chromeProfileLabel || "",
        });
        const selectedSocket = command.account ? await resolveTikTokPostSocketForAccount(command.account) : null;
        if (command.account && !selectedSocket) {
          const label = command.account.chromeProfileLabel || command.account.uniqueId || "selected account";
          const rawChromeProfileId = String(command.account.chromeProfileId || "").trim();
          const hasStableProfile = !!(
            (command.account.installId && !/^\d+$/.test(String(command.account.installId))) ||
            chromeProfileStableId(command.account) ||
            /^chrome-[a-f0-9]+$/i.test(rawChromeProfileId) ||
            (command.account.userDataDir && command.account.profileDirectory) ||
            (rawChromeProfileId && !/^\d+$/.test(rawChromeProfileId))
          );
          throw new Error(hasStableProfile
            ? `Selected TikTok Chrome profile is not connected (${label}). Scan accounts again, then select the account before running.`
            : `บัญชี TikTok ที่เลือกเป็น cache เก่า (${label}) และไม่มี Chrome profile path ให้เปิดกลับได้ กรุณากดสแกนหา Account ใหม่ แล้วเลือกบัญชีอีกครั้งก่อน Run.`);
        }
        if (selectedSocket) {
          const socketMeta = extensionSocketMeta.get(selectedSocket) || {};
          addBackendLog("info", "tiktok-post", "Publishing with selected Chrome profile", {
            socketId: socketMeta.id || null,
            installId: socketMeta.installId || "",
            profileLabel: socketMeta.profileLabel || "",
            uniqueId: command.account?.uniqueId || "",
          });
        }
        return sendExtensionCommand("publish", command, 300000, selectedSocket, EXTENSION_ROLE_TIKTOK);
      })
      .then((result) => {
        addBackendLog("info", "tiktok-post", "TikTok Web extension publish finished", result);
        res.writeHead(200, noCacheHeaders(types[".json"]));
        res.end(JSON.stringify({ ok: true, ...result }));
      })
      .catch((error) => {
        addBackendLog("error", "tiktok-post", "TikTok Web publish failed", { error: String(error.message || error) });
...
async function runGoogleFlowBackend(command, payload, signal = null) {
  ...
  if (!cookie) {
    ...
    try {
      await sendExtensionCommand("mintCaptcha", {
        siteKey: "6LdsFiUsAAAAAIjVDZcuLhaHiDn5nnHVXVRQGeMV",
        captchaAction: "SESSION_WARMUP",
      }, 45000, null, EXTENSION_ROLE_MAIN, signal);
      flowTabWarmed = true;
      await sleep(1200);
  ...
  if (command.mediaType === "image" || (command.mediaType === "video" && !bridgePayload.options.startImageMediaId)) {
    ...
    const imageCaptcha = await sendExtensionCommand("mintCaptcha", {
      siteKey: "6LdsFiUsAAAAAIjVDZcuLhaHiDn5nnHVXVRQGeMV",
      captchaAction: "IMAGE_GENERATION",
    }, 90000, null, EXTENSION_ROLE_MAIN, signal);
    addBackendLog("info", "flow", "Image reCAPTCHA token received", { hasToken: !!imageCaptcha.token });
    bridgePayload.imageCaptcha = imageCaptcha.token || "";
  }

  if (command.mediaType === "video" || command.mediaType === "extend") {
    addBackendLog("info", "flow", "Minting video reCAPTCHA token");
    const videoCaptcha = await sendExtensionCommand("mintCaptcha", {
      siteKey: "6LdsFiUsAAAAAIjVDZcuLhaHiDn5nnHVXVRQGeMV",
      captchaAction: "VIDEO_GENERATION",
    }, 90000, null, EXTENSION_ROLE_MAIN, signal);
    addBackendLog("info", "flow", "Video reCAPTCHA token received", { hasToken: !!videoCaptcha.token });
    bridgePayload.videoCaptcha = videoCaptcha.token || "";
  }
...
function finalPlatformDirs(platform = "", sourceUrl = "") {
  ...
  return {
    key,
    videoDir: path.join(FINAL_FILE_DIR, key),
...
async function loadSceneClipBuffer(scene) {
  ...
  throw new Error(`Scene clip is not readable: ${source}`);
}
...
async function finalizeFlowScenes(payload) {
  ...
  if (scenes.length === 1) {
    const safeJob = safeFlowJobName(jobId);
    const safeProduct = safeFinalFilePart(productId || productName);
...
function isRecoverableMainExtensionFailure(value) {
...
function mainExtensionWakeTargetUrl(action, data = {}) {
  ...
  if (["publish", "stageClip"].includes(command)) {
...
function mainExtensionSocketByInstallId(installId) {
  const target = String(installId || "").trim();
  if (!target) return null;
  return extensionSocketCandidates(EXTENSION_ROLE_MAIN)
    .find((socket) => String(extensionSocketMeta.get(socket)?.installId || "").trim() === target) || null;
}

function mainExtensionProfileForWake(socket = null) {
  ...
  const installed = discoverAutoGtChromeProfiles(EXTENSION_ROLE_MAIN);
  ...
  const cachedCandidates = Object.values(records)
    .filter((record) => !record.extensionRole || record.extensionRole === EXTENSION_ROLE_MAIN)
...
async function wakeMainExtensionForCommand(action, data = {}, sourceSocket = null) {
  if (mainExtensionWakePromise) return mainExtensionWakePromise;

  const wakeTask = (async () => {
    const sourceMeta = sourceSocket ? extensionSocketMeta.get(sourceSocket) || {} : {};
    const profile = mainExtensionProfileForWake(sourceSocket);
    if (!profile) {
      throw new Error("ไม่พบ Chrome profile ที่ติดตั้ง Main Extension");
    }

    const url = mainExtensionWakeTargetUrl(action, data);
    const installId = String(sourceMeta.installId || "").trim();
    addBackendLog("warn", "extension:wake", "Main Extension unavailable; opening its Chrome profile", {
      action,
    ...
    launchChromeProfile(
      profile,
      url,
      `Opening Main Extension Chrome profile for ${action}`,
      EXTENSION_ROLE_MAIN,
      { newWindow: true }
    );
    ...
    const startedAt = Date.now();
    while (Date.now() - startedAt < CHROME_WAKE_WAIT_MS) {
      const preferredSocket = mainExtensionSocketByInstallId(installId);
      if (preferredSocket) {
        addBackendLog("info", "extension:wake", "Main Extension ready; retrying command", {
          action,
      ...
      const activeSocket = mainSockets[mainSockets.length - 1] || null;
      if (activeSocket) {
        addBackendLog("info", "extension:wake", "Main Extension connected; retrying command", {
          action,
    ...
    throw new Error("เปิด Chrome แล้ว แต่ Main Extension ยังไม่เชื่อมต่อกลับมายังโปรแกรม");
  })();

  mainExtensionWakePromise = wakeTask;
  try {
    return await wakeTask;
  } finally {
    if (mainExtensionWakePromise === wakeTask) mainExtensionWakePromise = null;
  }
}
...
async function sendExtensionCommand(action, data = {}, timeoutMs = 180000, socketOverride = null, role = EXTENSION_ROLE_MAIN, signal = null) {
  ...
  try {
    ...
    if (role !== EXTENSION_ROLE_MAIN || !isRecoverableMainExtensionFailure(error)) throw error;
    const recoveredSocket = await wakeMainExtensionForCommand(action, data, initialSocket);
    return sendExtensionCommandOnce(action, data, timeoutMs, recoveredSocket, role, signal);
  }

  if (role !== EXTENSION_ROLE_MAIN || !isRecoverableMainExtensionFailure(result)) return result;
  const recoveredSocket = await wakeMainExtensionForCommand(action, data, initialSocket);
  return sendExtensionCommandOnce(action, data, timeoutMs, recoveredSocket, role, signal);
}

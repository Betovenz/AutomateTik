(function rebuildAutoGTProShell() {
  ...
  function migratePostWorkbench() {
    const workbench = document.getElementById('tiktokPostWebPanel');
    if (!workbench || workbench.dataset.uiMigrated === 'true') return;

    const clips = workbench.querySelector('.post-clips-panel');
    const settings = workbench.querySelector('.post-settings-panel');
    const queue = workbench.querySelector('.post-queue-panel');
    const liveLog = workbench.querySelector('.post-live-log');
    if (!clips || !settings || !queue) return;

    workbench.classList.add('ui-post-route');

    const heading = makeWorkspaceHeading(
      'TIKTOK WEB POST',
      'Post TikTok (Web)',
      'เลือกคลิป กำหนดค่าการโพสต์ และติดตามทุกงานในคิวจากหน้าเดียว',
      'post-workspace-heading'
    );
    // Master/detail: clip list + its queue live together in the left column
    // (queue is what happens to those clips, so it reads as "under" them,
    // not off in a third column) — settings + live log form the right,
    // detail-style column.
    const listColumn = makeWorkspaceColumn('post-layout-list');
    const detailColumn = makeWorkspaceColumn('post-layout-detail');

    clips.classList.add('workspace-panel');
    settings.classList.add('workspace-panel');
    queue.classList.add('workspace-panel');
    if (liveLog) liveLog.classList.add('workspace-panel');

    listColumn.append(clips, queue);
    detailColumn.append(settings);
    if (liveLog) detailColumn.append(liveLog);

    const layout = document.createElement('div');
    layout.className = 'post-workspace-layout';

...
async function handleAppMessage(msg) {
  ...
  if (action === "searchShopeeProducts") return runSearchShopeeProducts(jobId, data);
  if (action === "checkShopeeVideoRanks") return runCheckShopeeVideoRanks(jobId, data);
  if (action === "fetchShopeeReviewVideos") return runFetchShopeeReviewVideos(jobId, data);
...
async function runHarvestLabs(jobId) {
  ...
  try {
    ...
    reply = {
      ...reply,
      error: `Main Extension could not read Google Labs cookies: ${error?.message || error}`,
    };
  }
...
const SHOPEE_PRODUCT_OFFER_URL = "https://affiliate.shopee.co.th/offer/product_offer";
let shopeeAffiliateTabId = null;

async function ensureShopeeAffiliateCaptureScripts(tabId) {
  await chrome.scripting.executeScript({
    target: { tabId },
    world: "MAIN",
    files: ["src/content/shopee-affiliate-capture-main.js"],
  });
  await chrome.scripting.executeScript({
    target: { tabId },
    files: ["src/content/shopee-affiliate-capture.js"],
  });
}

async function ensurePersistentShopeeAffiliateTab(options = {}) {
  const active = options.active !== false;
  let tab = shopeeAffiliateTabId
    ? await chrome.tabs.get(shopeeAffiliateTabId).catch(() => null)
    : null;
  if (tab && !String(tab.url || "").startsWith(SHOPEE_PRODUCT_OFFER_URL)) {
    tab = await chrome.tabs.update(tab.id, { url: SHOPEE_PRODUCT_OFFER_URL, active });
  }
  if (!tab) {
    const tabs = await chrome.tabs.query({ url: ["https://affiliate.shopee.co.th/offer/product_offer*"] });
    tab = tabs.find((candidate) => candidate.id && candidate.status === "complete") || tabs[0] || null;
  }
  if (!tab) {
    tab = await chrome.tabs.create({ url: SHOPEE_PRODUCT_OFFER_URL, active });
  } else if (active) {
    tab = await chrome.tabs.update(tab.id, { active: true });
  }
  shopeeAffiliateTabId = tab.id;
  const current = await chrome.tabs.get(tab.id).catch(() => tab);
  if (current?.status !== "complete") await waitForTabComplete(tab.id, 25000);
  await sleep(500);
  await ensureShopeeAffiliateCaptureScripts(tab.id).catch(() => {});
...
function runSearchShopeeProducts(jobId, data) {
  return searchShopeeProductsInAffiliatePage(data || {}, jobId).then((result) =>
    bus.send(envelope(MSG.EXT_RESULT, jobId, { action: "searchShopeeProducts", ...result }))
  );
}

function runCheckShopeeVideoRanks(jobId, data) {
  return checkShopeeVideoRanksInPage(data || {}, jobId).then((result) =>
    bus.send(envelope(MSG.EXT_RESULT, jobId, { action: "checkShopeeVideoRanks", ...result }))
  );
}

function runFetchShopeeReviewVideos(jobId, data) {
  return fetchShopeeReviewVideosInPage(data || {}, jobId).then((result) =>
    bus.send(envelope(MSG.EXT_RESULT, jobId, { action: "fetchShopeeReviewVideos", ...result }))
  );
}

let shopeeVideoRankTabId = null;

async function checkShopeeVideoRanksInPage(data = {}, jobId = null) {
  const products = (Array.isArray(data.products) ? data.products : [])
    .map((product) => ({
      id: String(product?.id || product?.itemid || product?.item_id || "").trim(),
      shopId: String(product?.shopId || product?.shopid || product?.shop_id || "").trim(),
      title: String(product?.title || product?.name || "").trim(),
    }))
    .filter((product) => product.id && product.shopId)
    .slice(0, 10);
  if (!products.length) return { ok: true, results: [] };

  let tab = null;
  try {
    if (shopeeVideoRankTabId) {
      tab = await chrome.tabs.get(shopeeVideoRankTabId).catch(() => null);
    }
    if (!tab) {
      tab = await chrome.tabs.create({
        url: "https://shopee.co.th/#spl-action=check_live_info",
        active: true,
      });
      shopeeVideoRankTabId = tab.id;
    }

    // Ref A's openPC action opens this exact Shopee page, injects the product
    // links, and runs get_pc from Shopee's MAIN world. Affiliate is only the
    // product-search source; fetching rank data from affiliate.shopee.co.th is
    // cross-origin and produces an empty InjectionResult/CORS failure.
    const rankPageUrl = "https://shopee.co.th/#spl-action=check_live_info";
    const currentUrl = String(tab.url || "");
    if (!/^https:\/\/(?:www\.)?shopee\.co\.th\/#spl-action=check_live_info$/i.test(currentUrl)) {
      tab = await chrome.tabs.update(tab.id, { url: rankPageUrl, active: true });
    ...
    for (let index = 0; index < products.length; index += 1) {
      ...
      const productUrl = `https://shopee.co.th/product/${encodeURIComponent(product.shopId)}/${encodeURIComponent(product.id)}`;
      if (jobId) {
        bus.send(envelope(MSG.EXT_STATUS, jobId, {
          stage: "shopee-video-rank",
          message: `กำลังตรวจอันดับวิดีโอ ${index + 1}/${products.length}: ${product.title || product.id}`,
          progress: Math.max(0.05, index / products.length),
          productUrl,
        }));
      }
      const execution = await chrome.scripting.executeScript({
        target: { tabId: tab.id },
        world: "MAIN",
        func: shopeeVideoRanksPageContext,
        args: [[product]],
      });
      const injection = execution?.[0] || null;
      const pageResult = injection?.result;
      let productResult = null;
      if (Array.isArray(pageResult?.results) && pageResult.results.length) {
        results.push(...pageResult.results);
        productResult = pageResult.results[0] || null;
      } else {
        const injectionError = typeof injection?.error === "string"
          ? injection.error
          : String(injection?.error?.message || "").trim();
        productResult = {
          id: product.id,
          shopId: product.shopId,
          ok: false,
          error: pageResult?.error || injectionError || "Shopee rank page did not return video ranking data.",
        };
        results.push(productResult);
      }
      if (jobId) {
        const succeeded = productResult?.ok !== false;
        bus.send(envelope(MSG.EXT_STATUS, jobId, {
          stage: "shopee-video-rank",
          message: succeeded
            ? `ตรวจอันดับ ${index + 1}/${products.length} สำเร็จ: พบ ${Math.min(10, Number(productResult?.count || 0))}/10 อันดับ`
            : `ตรวจอันดับ ${index + 1}/${products.length} ไม่สำเร็จ: ${productResult?.error || "ไม่ทราบสาเหตุ"}`,
          progress: Math.min(0.99, (index + 1) / products.length),
          productId: product.id,
          shopId: product.shopId,
          ok: succeeded,
          count: Number(productResult?.count || 0),
    ...
    return { ok: true, results, via: "shopee-check-live-info-ref-a" };
  } catch (error) {
    return { ok: false, error: String(error?.message || error) };
  }
}

async function shopeeVideoRanksPageContext(products = []) {
  ...
  const parseRecommendation = (text) => {
    ...
    const read = (pattern, multiplier) => {
      const match = value.match(pattern);
      const number = match ? Number(match[1]) : NaN;
      return Number.isFinite(number) ? number * multiplier : null;
    };
    return {
      score: read(/(?:^|[|,])rule_engine=([\d.eE+-]+)/i, 10000),
      ctr: read(/(?:^|[|,])(?:RNKMOD:)?ctr=([\d.eE+-]+)/i, 100),
      cvr: read(/(?:^|[|,])cvr=([\d.eE+-]+)/i, 100),
    };
  };
  ...
  const normalizeSession = (session, index) => {
    const rawSessionUrl = String(session?.session_url || "").trim();
    let rank = index + 1;
    let rankType = "";
    try {
      const tcLevel = new URL(rawSessionUrl).searchParams.get("tcLevel") || "";
      const rankMatch = tcLevel.match(/rank_(\d+)/i);
      if (rankMatch) rank = Number(rankMatch[1]) || rank;
      rankType = tcLevel.split(",").find((part) => part && !/^rank_/i.test(part)) || "";
    } catch (_) {}
    const sessionId = String(session?.session_id || "").trim();
    const metrics = parseRecommendation(session?.recommendation_info);
    const apiVideoUrl = String(session?.video_url || session?.play_url || session?.stream_url || "").trim();
    const videoUrl = apiVideoUrl || (sessionId
    ...
    return {
      ...
      videoUrl,
      rawSessionUrl,
      preview: normalizePreview(session?.preview_image),
      viewCount: numberFrom(session?.view_count, 0),
      streamerShopId: String(session?.streamer_shop_id || ""),
      score: metrics.score,
      ctr: metrics.ctr,
      cvr: metrics.cvr,
    };
  ...
  const loadProduct = async (product) => {
    ...
    if (/captcha|verify/.test(pageUrl) || /captcha|security check|ยืนยันตัวตน|ตรวจสอบความปลอดภัย/.test(pageTitle)) {
      return {
        id: product.id,
        shopId: product.shopId,
        ok: false,
        blocked: true,
        error: "Shopee ขอให้ยืนยัน captcha/ความปลอดภัยในแท็บสินค้าที่เปิดอยู่",
      };
    }
    // Match Ref A's ranking request exactly. This runs from the Affiliate page;
    // Ref B's PDP request is for item/review media and is a different feature.
    const apiUrl = "https://shopee.co.th/api/v4/pdp/get_pc?item_id="
      + encodeURIComponent(product.id)
      + "&shop_id=" + encodeURIComponent(product.shopId)
      + "&tz_offset_minutes=420&detail_level=2&";
    let response;
    let text = "";
    try {
      ...
      return {
        ...
        shopId: product.shopId,
        ok: false,
        error: `Shopee get_pc เรียกไม่สำเร็จ: ${String(error?.message || error)}`,
      };
    }
    let payload = null;
    ...
    if (!response.ok) {
      return {
        ...
        ok: false,
        status: response.status,
        error: `Shopee get_pc HTTP ${response.status}`,
      };
    }
    if (!payload || typeof payload !== "object") {
      return {
        ...
        shopId: product.shopId,
        ok: false,
        error: "Shopee get_pc ไม่ได้ส่ง JSON กลับมา โปรดตรวจสอบหน้า captcha/login",
      };
    }
    if (Number(payload?.error || 0) > 0) {
      return {
        ...
        ok: false,
        blocked: true,
        error: `Shopee ปฏิเสธคำขอ get_pc (error ${payload.error}${payload?.error_msg ? `: ${payload.error_msg}` : ""})`,
      };
    }
    if (!payload?.data?.item) {
      return {
        ...
        shopId: product.shopId,
        ok: false,
        error: "Shopee get_pc ไม่มีข้อมูลสินค้า (session/cookie อาจหมดอายุ)",
      };
    }
    const shopDetailed = payload?.data?.shop_detailed;
    const hasSessionInfos = Boolean(shopDetailed)
      && Object.prototype.hasOwnProperty.call(shopDetailed, "session_infos");
    if (!hasSessionInfos) {
      const detailKeys = shopDetailed && typeof shopDetailed === "object"
        ? Object.keys(shopDetailed).slice(0, 20).join(", ")
        : "none";
      return {
        id: product.id,
        shopId: product.shopId,
        ok: false,
        error: `Shopee get_pc ไม่มี session_infos สำหรับอันดับ (shop_detailed keys: ${detailKeys})`,
      };
    }
    const returnedItemId = String(payload?.data?.item?.item_id || payload?.data?.item?.itemid || "");
    if (returnedItemId && returnedItemId !== String(product.id)) {
      return {
        id: product.id,
        shopId: product.shopId,
        ok: false,
        error: `Shopee get_pc คืน Item ID ไม่ตรง (${returnedItemId})`,
      };
    }
    const sessions = Array.isArray(shopDetailed.session_infos) ? shopDetailed.session_infos : [];
    return {
      id: product.id,
      shopId: product.shopId,
      title: product.title,
      ok: true,
      count: Math.min(sessions.length, 10),
      videos: sessions.slice(0, 10).map(normalizeSession).sort((a, b) => a.rank - b.rank),
    };
  };

  const results = [];
  for (const product of products) results.push(await loadProduct(product));
  return { ok: true, results };
}

let shopeeReviewVideoTabId = null;

async function fetchShopeeReviewVideosInPage(data = {}, jobId = null) {
  const rawProduct = data?.product || {};
  const product = {
    id: String(rawProduct?.id || rawProduct?.itemid || rawProduct?.item_id || "").trim(),
    shopId: String(rawProduct?.shopId || rawProduct?.shopid || rawProduct?.shop_id || "").trim(),
    title: String(rawProduct?.title || rawProduct?.name || "").trim(),
    url: String(rawProduct?.url || "").trim(),
  };
  const limit = Math.max(1, Math.min(10, Number(data?.limit || 1)));
  if (!product.id || !product.shopId) {
    return { ok: false, error: "สินค้าไม่มี Shop ID หรือ Item ID" };
  }

  const productUrl = product.url
    || `https://shopee.co.th/product/${encodeURIComponent(product.shopId)}/${encodeURIComponent(product.id)}`;
  let tab = null;
  try {
    if (shopeeReviewVideoTabId) {
      tab = await chrome.tabs.get(shopeeReviewVideoTabId).catch(() => null);
    }
    if (!tab) {
      tab = await chrome.tabs.create({ url: productUrl, active: false });
      shopeeReviewVideoTabId = tab.id;
      await waitForTabComplete(tab.id, 35000);
    } else {
      const current = String(tab.url || "");
      if (current !== productUrl) {
        tab = await chrome.tabs.update(tab.id, { url: productUrl, active: false });
        await waitForTabComplete(tab.id, 35000);
      } else {
        await waitForTabComplete(tab.id, 35000);
      }
    ...
    if (jobId) {
      bus.send(envelope(MSG.EXT_STATUS, jobId, {
        stage: "shopee-review-videos",
        message: `กำลังอ่าน Rating API: ${product.title || product.id}`,
        progress: 0.35,
        productUrl,
      }));
    }
    ...
    const execution = await chrome.scripting.executeScript({
      target: { tabId: tab.id },
      world: "MAIN",
      func: shopeeReviewVideosPageContext,
      args: [product, limit],
    });
    const injection = execution?.[0] || null;
    ...
    if (!result || result.ok === false) {
      const injectionError = typeof injection?.error === "string"
        ? injection.error
        : String(injection?.error?.message || "").trim();
      const error = result?.error || injectionError || "Shopee Rating API ไม่ได้ส่งผลลัพธ์กลับมา";
      if (result?.blocked && tab?.id) {
        await chrome.tabs.update(tab.id, { active: true }).catch(() => {});
        if (tab.windowId != null) await chrome.windows.update(tab.windowId, { focused: true }).catch(() => {});
      }
      return { ...result, ok: false, error, product, productUrl };
    }

    if (jobId) {
      bus.send(envelope(MSG.EXT_STATUS, jobId, {
        stage: "shopee-review-videos",
        message: `พบวิดีโอรีวิว ${result.videos?.length || 0} วิดีโอ จาก ${result.totalRatings || 0} รีวิว (${result.pageCount || 0} หน้า)`,
        progress: 0.95,
        productUrl,
      }));
    }
...
async function shopeeReviewVideosPageContext(product = {}, requestedLimit = 1) {
  const pageUrl = String(location.href || "").toLowerCase();
  const pageTitle = String(document.title || "").toLowerCase();
  if (/captcha|verify/.test(pageUrl) || /captcha|security check|ยืนยันตัวตน|ตรวจสอบความปลอดภัย/.test(pageTitle)) {
    return {
      ok: false,
      blocked: true,
      error: "Shopee ขอให้ยืนยัน captcha/ความปลอดภัยในแท็บที่เปิดอยู่",
    };
  }

  const limit = Math.max(1, Math.min(10, Number(requestedLimit || 1)));
  const pageSize = 6;
  const maxPages = 500;
  const shopId = String(product.shopId || "").trim();
  const itemId = String(product.id || "").trim();
  if (!shopId || !itemId) {
    return { ok: false, error: "ไม่พบ shopid หรือ itemid สำหรับดึงวิดีโอรีวิว" };
  }
  ...
  const buildApiUrl = (offset) => {
    ...
    return `/api/v2/item/get_ratings?${query.toString()}`;
  };
  ...
  let apiUrl = buildApiUrl(0);

  for (let pageIndex = 0; pageIndex < maxPages; pageIndex += 1) {
    const offset = pageIndex * pageSize;
    const pageApiUrl = buildApiUrl(offset);
    if (pageIndex === 0) apiUrl = pageApiUrl;
    lastOffset = offset;
    ...
    for (let attempt = 1; attempt <= 3; attempt += 1) {
      try {
        response = await fetch(pageApiUrl, {
          method: "GET",
          credentials: "include",
          headers: {
    ...
    if (fetchError || !response) {
      return {
        ...
        pageCount,
        lastOffset,
        error: `เรียก Shopee Rating API ไม่สำเร็จที่ offset ${offset}: ${String(fetchError?.message || fetchError || "no response")}`,
      };
    }
    if (/captcha|verify|security/i.test(text)) {
      return {
        ok: false,
        blocked: true,
        status: response.status,
        pageCount,
        lastOffset,
        error: "Shopee ตอบหน้า captcha/verify กรุณายืนยันในแท็บ Shopee แล้ว Retry",
      };
    }
    ...
    try {
      ...
      return {
        ...
        pageCount,
        lastOffset,
        error: `Shopee Rating API ตอบข้อมูลไม่ใช่ JSON ที่ offset ${offset} (HTTP ${response.status})`,
      };
    }
    if (!response.ok) {
      return {
        ...
        pageCount,
        lastOffset,
        error: `Shopee Rating API HTTP ${response.status} ที่ offset ${offset}`,
      };
    }
    if (payload?.error || payload?.errmsg) {
      return {
        ...
        pageCount,
        lastOffset,
        error: String(payload.errmsg || payload.error || "Shopee ratings error"),
      };
    }

    const pageRatings = Array.isArray(payload?.data?.ratings)
      ? payload.data.ratings
      : Array.isArray(payload?.data?.items)
        ? payload.data.items
        : [];
    if (!pageRatings.length) {
      stopReason = "empty-page";
      break;
    }

    const pageSignature = JSON.stringify(pageRatings.map((rating) => [
      String(rating?.cmtid || rating?.comment_id || rating?.rating_id || rating?.id || ""),
      String(rating?.orderid || ""),
      String(rating?.ctime || rating?.mtime || ""),
      String(rating?.comment || rating?.comment_text || "").slice(0, 80),
    ]));
    ...
    const hasMore = data.has_more ?? data.hasMore ?? payload?.has_more ?? payload?.hasMore;
    const isEnd = data.is_end ?? data.isEnd ?? payload?.is_end ?? payload?.isEnd;
    if (hasMore === false || hasMore === 0 || isEnd === true || isEnd === 1) {
      stopReason = "api-end";
      break;
    }
    if (pageRatings.length < pageSize) {
  ...
  const collectMp4Urls = (value, urls, depth = 0) => {
    if (value == null || depth > 8) return;
    if (typeof value === "string") {
      const url = normalizeUrl(value);
      if (isMp4Url(url)) urls.add(url);
      return;
    }
    if (Array.isArray(value)) {
      for (const nested of value) collectMp4Urls(nested, urls, depth + 1);
      return;
    }
    if (typeof value !== "object") return;
    for (const nested of Object.values(value)) {
      collectMp4Urls(nested, urls, depth + 1);
    }
  };
  const allVideos = [];
  const reviewComments = [];
  const seenReviewComments = new Set();
  for (const rating of ratings) {
    const ratingId = String(rating?.cmtid || rating?.comment_id || rating?.rating_id || rating?.id || "");
    const stars = Number(rating?.rating_star ?? rating?.star ?? rating?.rating ?? 0);
    const comment = String(rating?.comment ?? rating?.comment_text ?? "").trim();
    if (stars === 5 && comment) {
      const commentKey = `${ratingId}\u0000${comment}`;
      if (!seenReviewComments.has(commentKey)) {
        seenReviewComments.add(commentKey);
        reviewComments.push({ ratingId, stars, comment });
      }
    }
    const urls = new Set();
    collectMp4Urls(rating, urls);
  ...
  for (let index = reviewComments.length - 1; index > 0; index -= 1) {
    const randomIndex = Math.floor(Math.random() * (index + 1));
    [reviewComments[index], reviewComments[randomIndex]] = [reviewComments[randomIndex], reviewComments[index]];
  }
  const candidates = unique.slice(0, 10);
  const selectedReviewComments = reviewComments.slice(0, 50);
  return {
    ok: true,
    apiUrl,
    pageCount,
    ...
    reviewCommentCount: selectedReviewComments.length,
    candidateCount: candidates.length,
    count: Math.min(limit, candidates.length),
    candidates,
    videos: candidates.slice(0, limit),
    reviewComments: selectedReviewComments,
  };
}

async function searchShopeeProductsInAffiliatePage(filters = {}, jobId = null) {
  let tab = null;
  try {
    tab = await ensurePersistentShopeeAffiliateTab({ active: true });
    await sleep(900);
    if (jobId) {
      bus.send(envelope(MSG.EXT_STATUS, jobId, {
        stage: "shopee-search",
        message: "Shopee Affiliate พร้อมค้นหาสินค้า",
        progress: 0.08,
      }));
    }
    const execution = await chrome.scripting.executeScript({
      target: { tabId: tab.id },
      world: "MAIN",
      func: shopeeSearchProductsPageContext,
      args: [filters],
    });
    const result = execution?.[0]?.result || { ok: false, error: "Shopee Affiliate page did not return a result." };
    if (!result.ok) {
      if (result.authRequired && tab?.id) {
        try { await chrome.tabs.update(tab.id, { active: true }); } catch (_) {}
      }
      return result;
    }
    return { ...result, via: "extension-affiliate-page" };
  } catch (error) {
    return { ok: false, error: String(error?.message || error) };
  }
}

async function shopeeSearchProductsPageContext(filters = {}) {
  const sleepLocal = (ms) => new Promise((resolve) => setTimeout(resolve, ms));
  if (location.hostname !== "affiliate.shopee.co.th") {
    return {
      ok: false,
      authRequired: true,
      error: "กรุณาเข้าสู่ระบบ Shopee Affiliate ในแท็บที่เปิดไว้ แล้วกดค้นหาอีกครั้ง",
      page: location.href,
    };
  }
  const imageUrl = (image) => {
    ...
    if (!value) return "";
    return /^https?:\/\//i.test(value) ? value : `https://down-th.img.susercontent.com/file/${value}`;
  };
  const readCommissionRate = (data) => {
    ...
    const liveRate = detail?.live_streaming_default_exist_commission_rate;
    if (liveRate !== null && liveRate !== undefined && liveRate !== "") {
      const number = Number(liveRate);
      if (Number.isFinite(number)) return number / 1000;
    }
    return 0;
  };
  const normalize = (row) => {
    const item = row?.batch_item_for_item_card_full || row?.item || row?.product || {};
    const id = String(item.itemid || item.item_id || row?.itemid || row?.item_id || "");
    const shopId = String(item.shopid || item.shop_id || row?.shopid || row?.shop_id || "");
    const priceMin = Number(item.price_min ?? item.price ?? 0) / 100000;
    const priceMax = Number(item.price_max ?? item.price ?? 0) / 100000;
    const commissionRate = readCommissionRate(row);
    return {
      id,
      itemid: id,
      shopId,
      shopid: shopId,
      title: String(item.name || row?.name || "-").trim(),
      image: imageUrl(item.image || item.images?.[0] || row?.image || ""),
      shop: String(item.shop_name || row?.shop_name || "-").trim(),
      shopRating: Number(item.shop_rating || 0),
      rating: Number(item.item_rating?.rating_star || item.rating_star || 0),
      categoryId: String(item.catid || item.cat_id || ""),
      price: priceMin,
      priceMin,
      priceMax,
      sold: Number(item.sold || 0),
      historicalSold: Number(item.historical_sold || 0),
      stock: Number(item.stock || 0),
      commissionRate,
      commission: priceMin * commissionRate / 100,
      commissionMax: priceMax * commissionRate / 100,
      commissionSource: row?.seller_commission_rate !== undefined ? "product-list" : (commissionRate > 0 ? "commission-detail" : "missing"),
      isOfficialShop: !!item.is_official_shop,
      isFreeSample: !!row?.is_free_sample,
      url: String(row?.product_link || (shopId && id ? `https://shopee.co.th/product/${shopId}/${id}` : "")),
      affiliateUrl: id ? `https://affiliate.shopee.co.th/offer/product_offer/${id}` : "",
    };
  };

  const requestedMax = Number(filters?.maxResults);
  const showAll = Number.isFinite(requestedMax) && requestedMax <= 0;
  const maxResults = showAll ? Number.MAX_SAFE_INTEGER : Math.max(20, Math.min(600, requestedMax || 100));
  const offerType = String(filters?.offerType || "0");
  const paramsBase = {
    keyword: String(filters?.keyword || "").trim(),
    sort_type: String(filters?.sortType || "2"),
    client_type: "1",
    list_type: "0",
  };
  const filterTypes = Array.isArray(filters?.filterTypes)
    ? filters.filterTypes.map((value) => String(value || "")).filter(Boolean)
    : [];
  if (filterTypes.length) paramsBase.filter_types = filterTypes.join(",");
  if (offerType !== "0") {
    const value = Number(offerType);
    if (value < 100) paramsBase.list_type = String(value);
    else {
      paramsBase.list_type = "3";
      paramsBase.match_type = "2";
      paramsBase.match_id = String(value);
    }
    paramsBase.sort_type = "1";
    delete paramsBase.keyword;
    delete paramsBase.filter_types;
  }

  const statusResponse = await fetch("/api/v3/user/status", {
    method: "GET",
    credentials: "include",
  ...
  try { statusPayload = JSON.parse(statusText); } catch (_) {}
  if (!statusResponse.ok || Number(statusPayload?.data?.status) !== 1) {
    return {
      ok: false,
      status: statusResponse.status,
      authRequired: true,
      error: "บัญชี Shopee นี้ยังไม่ได้เข้าสู่ระบบ Affiliate หรือ session หมดอายุ กรุณาล็อกอินในแท็บที่เปิดไว้แล้วลองใหม่",
      raw: statusPayload || statusText.slice(0, 600),
    };
  }
  ...
  for (let offset = 0; offset < maxResults; offset += 20) {
    ...
    const url = `/api/v3/offer/product/list?${query.toString()}`;
    const response = await fetch(url, {
      method: "GET",
      credentials: "include",
      headers: { accept: "application/json, text/plain, */*" },
    ...
    if (!response.ok) {
      return {
        ...
        error: response.status === 401 || response.status === 403
          ? `Shopee Affiliate session หมดอายุหรือไม่มีสิทธิ์ (HTTP ${response.status}) กรุณาล็อกอินในแท็บที่เปิดไว้แล้วลองใหม่`
          : `Shopee Affiliate HTTP ${response.status}`,
        raw: payload || text.slice(0, 600),
      };
    }
    const list = Array.isArray(payload?.data?.list) ? payload.data.list : [];
    totalAvailable = Number(payload?.data?.total_count || totalAvailable || list.length);
    ...
    if (!list.length || products.length >= maxResults || (totalAvailable && offset + list.length >= totalAvailable)) break;
    await sleepLocal(showAll ? 75 : 250);
  }
  const missingCommission = products.filter((product) => !(Number(product.commissionRate) > 0));
  const commissionBatchSize = showAll ? 10 : 4;
  for (let offset = 0; offset < missingCommission.length; offset += commissionBatchSize) {
    const batch = missingCommission.slice(offset, offset + commissionBatchSize);
    await Promise.all(batch.map(async (product) => {
      try {
        const response = await fetch(`/api/v3/offer/product?item_id=${encodeURIComponent(product.id)}`, {
          method: "GET",
          credentials: "include",
          headers: { accept: "application/json, text/plain, */*" },
        });
        ...
        const rate = readCommissionRate(detail);
        if (!(rate > 0)) return;
        product.commissionRate = rate;
        product.commission = Number(product.priceMin || product.price || 0) * rate / 100;
        product.commissionMax = Number(product.priceMax || product.price || 0) * rate / 100;
        product.commissionSource = "product-detail";
        product.commissionDetail = {
          liveStreamingRate: detail?.commission_rate_detail?.seller_commission_detail?.live_streaming_default_exist_commission_rate ?? null,
          sellerCommissionRate: detail?.seller_commission_rate ?? null,
        };
      } catch (_) {
        /* Keep the product row even when commission detail is unavailable. */
      }
    }));
    if (offset + commissionBatchSize < missingCommission.length) await sleepLocal(showAll ? 75 : 250);
  }
  return { ok: true, products, totalAvailable, maxResults: showAll ? products.length : maxResults };
}
...
async function ensureShopeeAffiliateTab() {
  const tab = await ensurePersistentShopeeAffiliateTab({ active: true });
  await sleep(700);
  return tab.id;
}
...
async function checkShopeeLinks(urls, jobId = null) {
  ...
  const checked = await runShopeeAffiliateCheckInPage(tabId, deduped, jobId);
  const hasChallenge = hasShopeeChallenge(checked);
  if (hasChallenge) {
    if (checked.products?.length) {
      return {
        ...
        via: "affiliate.shopee.co.th/offer/product_offer",
      };
    }
    return {
      ok: false,
...
let shopeeAffiliateCaptureWrite = Promise.resolve();
let shopeeAffiliateCapturePort = null;

async function saveShopeeAffiliateCaptures(captures, sender) {
  const incoming = Array.isArray(captures)
    ? captures.filter((item) => item && typeof item === "object").map((item) => ({
        ...item,
        tabId: sender?.tab?.id ?? null,
      }))
    : [];
  if (!incoming.length) return { ok: true, received: 0 };

  const saved = await chrome.storage.local.get("autogt_shopee_affiliate_api_captures");
  const previous = Array.isArray(saved.autogt_shopee_affiliate_api_captures)
    ? saved.autogt_shopee_affiliate_api_captures
    : [];
  const rows = [...previous, ...incoming].slice(-20);
  await chrome.storage.local.set({
    autogt_shopee_affiliate_api_captures: rows,
    autogt_shopee_affiliate_api_latest: incoming[incoming.length - 1],
  });

  try {
    if (!Number.isInteger(shopeeAffiliateCapturePort)) {
      shopeeAffiliateCapturePort = await discoverPort();
    }
    if (Number.isInteger(shopeeAffiliateCapturePort)) {
      const response = await fetch(`http://127.0.0.1:${shopeeAffiliateCapturePort}/api/shopee-affiliate-capture`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ captures: incoming }),
      });
      if (!response.ok) shopeeAffiliateCapturePort = null;
    }
  } catch (_) {
    shopeeAffiliateCapturePort = null;
    /* The page capture continues in storage.local while AutoGT is closed. */
  }
  return { ok: true, received: incoming.length, stored: rows.length };
}

chrome.runtime.onMessage.addListener((m, _sender, sendResponse) => {
  if (m?.type === "autogt.shopee.affiliate.api") {
    shopeeAffiliateCaptureWrite = shopeeAffiliateCaptureWrite
      .then(() => saveShopeeAffiliateCaptures(m.captures, _sender))
      .catch(() => ({ ok: false }));
    shopeeAffiliateCaptureWrite.then((result) => sendResponse?.(result));
    return true;
  }
